...
class VersionInline(NestedStackedInline):
    model = Version
    readonly_fields = ('title', 'text', 'text_hash', 'language', 'date_added', 'date_last_seen', 'date_published', 'found_by',)
    extra = 0
...
class ArticleAdmin(NestedModelAdmin):
    ...
    list_display = ('get_url', 'title', 'get_authors', 'get_keywords', 'get_source_sites', 'get_source_twitters', 'language', 'date_added', 'date_published', 'link_options')
    ...
    def get_date_added(self, obj):
        return obj.version_set.last().date_added
